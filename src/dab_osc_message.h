...
#pragma mark OscMessage definition
    ...
    public:
        ...
        const _OscArg* operator[]( unsigned int pArgIndex ) const throw (Exception);
        const std::vector< _OscArg* >& arguments() const;
        
        template<typename Type> bool argumentMatch( unsigned int pArgIndex ) const throw (Exception);
        bool argumentMatch( unsigned int pArgIndex, OscType pOscType ) const throw (Exception);
        bool argumentMatch( const std::initializer_list<OscType>& pOscTypes ) const throw (Exception);
        unsigned int valueCount(unsigned int pArgIndex) const throw (Exception);
        template<typename Type> void get( unsigned int pArgIndex, Type& pValue ) const throw (Exception);
        template<typename Type> void get( unsigned int pArgIndex, Type* pValues ) const throw (Exception);
...
#pragma mark MultiOscMessage definition
    ...
    public:
        ...
        bool complete() const;
        std::shared_ptr<OscMessage> message() const throw (Exception);
        ...
        void setPartialMessage( unsigned int pIndex, std::shared_ptr<OscMessage> pPartialMessage ) throw (Exception);
...
#pragma mark OscMessage implementation
    ...
    template<typename Type>
    bool
    OscMessage::argumentMatch( unsigned int pArgIndex ) const throw (Exception)
    {
    ...
    template<typename Type>
    void
    OscMessage::get( unsigned int pArgIndex, Type& pValue ) const throw (Exception)
    {
        if( pArgIndex >= mArguments.size() ) throw ("OSC Error: requested argument " + std::to_string(pArgIndex) + " exceeds number of arguments available " + std::to_string( mArguments.size() ), __FILE__, __FUNCTION__, __LINE__  );
    ...
    template<typename Type>
    void
    OscMessage::get( unsigned int pArgIndex, Type* pValues ) const throw (Exception)
    {
        if( pArgIndex >= mArguments.size() ) throw ("OSC Error: requested argument " + std::to_string(pArgIndex) + " exceeds number of arguments available " + std::to_string( mArguments.size() ), __FILE__, __FUNCTION__, __LINE__  );
