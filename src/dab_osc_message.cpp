...
bool
OscMessage::argumentMatch( unsigned int pArgIndex, OscType pOscType ) const throw (Exception)
{
...
bool
OscMessage::argumentMatch( const std::initializer_list<OscType>& pOscTypes ) const throw (Exception)
{
...
unsigned int
OscMessage::valueCount(unsigned int pArgIndex) const throw (Exception)
{
    if( pArgIndex >= mArguments.size() ) throw ("OSC Error: requested argument " + std::to_string(pArgIndex) + " exceeds number of arguments available " + std::to_string( mArguments.size() ), __FILE__, __FUNCTION__, __LINE__  );
...
const _OscArg*
OscMessage::operator[]( unsigned int pArgIndex ) const throw (Exception)
{
    if( pArgIndex >= mArguments.size() ) throw ("OSC Error: requested argument " + std::to_string(pArgIndex) + " exceeds number of arguments available " + std::to_string( mArguments.size() ), __FILE__, __FUNCTION__, __LINE__  );
...
MultiOscMessage::message() const throw (Exception)
{
...
void
MultiOscMessage::setPartialMessage( unsigned int pIndex, std::shared_ptr<OscMessage> pPartialMessage ) throw (Exception)
{
