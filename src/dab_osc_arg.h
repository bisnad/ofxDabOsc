...
#pragma mark _OscArg definition
	...
	public:
		...
		template<typename Type> operator Type&() throw (Exception);
		template<typename Type> operator const Type&() const throw (Exception);
		template<typename Type> operator Type*() throw (Exception);
		template<typename Type> operator const Type*() const throw (Exception);

		virtual _OscArg* copy() const = 0;
		virtual _OscArg* copy(unsigned int pStartIndex, unsigned int pValueCount) const throw (Exception) = 0;
...
#pragma mark OscArg definition
	...
	public:
		...
		_OscArg* copy() const;
		_OscArg* copy(unsigned int pStartIndex, unsigned int pValueCount) const throw (Exception);
		void append(const _OscArg& pOscArg)  throw (Exception);
	...
	template<> _OscArg* OscArg<std::string>::copy(unsigned int pStartIndex, unsigned int pValueCount) const throw (Exception);
	template<> void OscArg<std::string>::append(const _OscArg& pArg) throw (Exception);
...
#pragma mark _OscArg implementation
	...
	template<typename Type>
	_OscArg::operator Type&() throw (Exception)
	{
	...
	template<typename Type>
	_OscArg::operator const Type&() const throw (Exception)
	{
		OscUtils& oscUtils = OscUtils::get();
		OscType pOscTape = oscUtils.oscType<Type>();
...
#pragma mark OscArg implementation
	...
	template<typename Type>
	_OscArg*
		OscArg<Type>::copy(unsigned int pStartIndex, unsigned int pValueCount) const throw(Exception)
	{
	...
	template<typename Type>
	void
		OscArg<Type>::append(const _OscArg& pArg) throw (Exception)
	{
